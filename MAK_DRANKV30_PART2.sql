--------------------------------------------------------
--  DDL for View MAK_DRANKV30_PART2
--------------------------------------------------------

  CREATE OR REPLACE VIEW "SCPOMGR"."MAK_DRANKV30_PART2" ("SECTION", "ITEM", "DEST", "SOURCE", "TRANSMODE", "EFF", "FACTOR", "ARRIVCAL", "MAJORSHIPQTY", "MINORSHIPQTY", "ENABLEDYNDEPSW", "SHRINKAGEFACTOR", "MAXSHIPQTY", "ABBR", "SOURCING", "DISC", "MAXLEADTIME", "MINLEADTIME", "PRIORITY", "ENABLESW", "YIELDFACTOR", "SUPPLYLEADTIME", "COSTPERCENTAGE", "SUPPLYTRANSFERCOST", "NONEWSUPPLYDATE", "SHIPCAL", "FF_TRIGGER_CONTROL", "PULLFORWARDDUR", "SPLITQTY", "LOADDUR", "UNLOADDUR", "REVIEWCAL", "USELOOKAHEADSW", "CONVENIENTSHIPQTY", "CONVENIENTADJUPPCT", "CONVENIENTOVERRIDETHRESHOLD", "ROUNDINGFACTOR", "ORDERGROUP", "ORDERGROUPMEMBER", "LOTSIZESENABLEDSW", "CONVENIENTADJDOWNPCT") AS 
  select distinct 'U_30_SRC_DAILY_PART2' section
      ,ranked_lanes.item
      , ranked_lanes.dest
      ,ranked_lanes.source
      ,'TRUCK' transmode
    ...
    from sourcing src
      ,
        ...
        *********************************************************************/
        (
        select all_lanes.item
          , all_lanes.dest
          , all_lanes.dest_pc
          , all_lanes.source
          ,all_lanes.source_pc
          , all_lanes.u_max_dist
          , all_lanes.u_max_src
          ,all_lanes.distance
          , dense_rank() over ( partition by all_lanes.item, all_lanes.dest order by cost_pallet, source asc ) as rank
        from
            /*********************************************************************
            ...
            **********************************************************************/
            (
            select
                /*+ use_hash(lane_cost, lanes) parallel (lane_cost,4) parallel (lanes,4) */
                lanes.item
              , lanes.dest
              , lanes.dest_pc
              , lanes.source
              ...
              , lanes.u_max_src
              , lane_cost.distance
              ,nvl(lane_cost.cost_pallet, 999) cost_pallet
              , lane_cost.direction
            ...
            from
              ...
              ,
                ...
                *****************************************************************/
                (
                select
                    /*+ use_hash(dest, source_sku, demand_group) parallel (dest,4)parallel (sku_source,4 ) */
                    ' ' direction
                  ,dest.item
                  ,dest.loc dest
                  ,dest.u_max_dist
                ...
                where dest.loc <> source_sku.loc
                    ...
                    and ( ( u_equipment_type='FB'
                    and source_sku.flatbed_status=1)
                    or (u_equipment_type <> 'FB') )
                ) lanes
