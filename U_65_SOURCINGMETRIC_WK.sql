--------------------------------------------------------
--  DDL for View U_65_SOURCINGMETRIC_WK
--------------------------------------------------------

  CREATE OR REPLACE VIEW "SCPOMGR"."U_65_SOURCINGMETRIC_WK" ("ITEM", "DEST", "SOURCE", "LOC_TYPE", "SOURCING", "CATEGORY", "DESCR", "TOTAL", "P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8", "P9", "P10", "P11", "P12", "P13", "P14", "P15", "P16", "P17", "P18", "P19", "P20", "P21", "P22", "P23", "P24", "P25", "P26", "DEST_DESCR", "DEST_CITY", "DEST_STATE", "SRC_DESCR", "SRC_CITY", "SRC_STATE") AS 
  select distinct item, dest, source, loc_type, sourcing, category, descr,
    ...
    dest_descr, dest_city, dest_state, src_descr, src_city, src_state 
    
from

    (select distinct c.item, c.dest, ld.descr dest_descr, ld.u_city dest_city, ld.u_state dest_state, c.source, l.descr src_descr, l.u_city src_city, l.u_state src_state, l.loc_type, c.sourcing, c.category, m.descr,  
    ...
    nvl (sum (case when eff = s.ohpost + 175 then round(value, 2) end), 0) p26
     from udt_sourcingmetric_wk c, metriccategory m, sku s,loc l, loc ld
    where     c.category = m.category
          and c.category in (418)
          and c.item = s.item
          and c.source = s.loc 
          and c.source = l.loc --and c.dest = '3000000003'
          and c.dest = ld.loc
    group by c.item, c.dest, c.source, l.loc_type, c.sourcing, c.category, m.descr, l.descr, l.u_city, l.u_state, ld.descr, ld.u_city, ld.u_state
)
